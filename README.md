...
It was observed that enabling hardware virtualization (BIOS settings) may lead to a situation when the GPU devices stall without progress but refered as 100% utilized by the 'nvidia-smi' utility. A software-only remedy (one that doesn't require hardware re-configuraion, or restart) is possible through setting the environment variable 'NCCL_P2P_DISABLE=1'. Alternatively, in order to achieve higher memory transfer troughput rates, one can disable BIOS virtualization options like "Virtualization Technology" and/or "VT-d".
